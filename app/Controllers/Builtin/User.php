...
class User extends \App\Controllers\BaseController
{
	...
	public function __construct() {
		...
		$this->addJs($this->config->baseURL . '/vendors/bootstrap-datepicker/js/bootstrap-datepicker.min.js');
		$this->addJs($this->config->baseURL . '/themes/modern/js/date-picker.js');
		$this->addJs($this->config->baseURL . '/themes/modern/builtin/js/user.js');
		$this->addJs($this->config->baseURL . '/themes/modern/builtin/js/image-upload.js');
		$this->addStyle($this->config->baseURL . '/vendors/bootstrap-datepicker/css/bootstrap-datepicker.min.css');
		
		$this->addJs ( $this->config->baseURL . '/vendors/jquery.select2/js/select2.full.min.js' );
		$this->addStyle ( $this->config->baseURL . '/vendors/jquery.select2/css/select2.min.css' );
		$this->addStyle ( $this->config->baseURL . '/vendors/jquery.select2/bootstrap-5-theme/select2-bootstrap-5-theme.min.css' );
	...
	public function getDataDT() {
		...
		helper('html');
		$avatar_path = ROOTPATH . '/images/user/';
		...
		foreach ($user['data'] as $key => &$val) {
			...
			$val['ignore_avatar'] = '<img src="'. $this->config->baseURL . 'images/user/' . $avatar . '">';
	...
	{		
		...
		if ($form_errors) {
			$result['message'] = ['status' => 'error', 'message' => $form_errors];
			$error = true;
		}
